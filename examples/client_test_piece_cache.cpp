...
// Global variables for display and control
...
// Flags for storage modes
bool disable_original_storage = false;
bool seed_from_cache = false;
bool use_ipfs_naming = false;

// Name mapping storage
std::unordered_map<std::string, std::unordered_map<std::string, std::string>> torrent_name_mappings;
...
bool is_absolute_path(std::string const& f)
{
    ...
    if (i < int(f.size()-1) && f[i] == ':' && (f[i+1] == '\\' || f[i+1] == '/')) return true;
    if (int(f.size()) >= 2 && f[0] == '\\' && f[1] == '\\') return true;
    return false;
#else
    if (f[0] == '/') return true;
    return false;
...
{
    using namespace lt;
    auto i = std::find_if(peers.begin(), peers.end(), [&addr](peer_info const& pi) { return pi.ip == addr; });
...
// Function to generate IPFS-style hash for a file or folder
std::string generate_ipfs_hash(const std::string& content)
{
...
// Function to find the torrent root directory
std::string find_torrent_root(const std::string& save_path, const std::string& info_hash) {
    // First try with the info_hash as directory name
    std::string torrent_dir = path_append(save_path, info_hash);
    if (std::filesystem::exists(torrent_dir)) {
        return torrent_dir;
    }

    // If not found, search through the save_path directory
...
// Function to rename files and folders to IPFS-style names
void rename_to_ipfs_names(const std::string& save_path, const std::string& info_hash, torrent_handle& h) {
    std::string torrent_dir = find_torrent_root(save_path, info_hash);
    if (torrent_dir.empty()) {
        log_message("Error: Could not find torrent directory for info_hash: " + info_hash);
        return;
    }

    log_message("Renaming files in " + torrent_dir + " to IPFS-style names");

    // Get the torrent info to map original names to IPFS names
    auto ti = h.torrent_file();
    if (!ti) {
        log_message("Error: Could not get torrent info");
        return;
    }

    // Clear any existing mappings for this torrent
    torrent_name_mappings[info_hash].clear();

    // Process each file in the torrent
    for (file_index_t i(0); i < ti->num_files(); ++i) {
        // Get the file path as a string
        std::string original_path;
        for (int j = 0; j < ti->files().file_path(i).size(); ++j) {
            if (j > 0) original_path += "/";
            original_path += ti->files().file_path(i)[j];
        }

        std::string file_path = path_append(torrent_dir, original_path);
        ...
        if (std::filesystem::exists(file_path)) {
            ...
            if (file) {
                ...
                std::string ipfs_name = generate_ipfs_hash(content);

                // Store the mapping
                torrent_name_mappings[info_hash][original_path] = ipfs_name;

                // Create parent directories if they don't exist
                std::filesystem::path parent_path = std::filesystem::path(path_append(torrent_dir, ipfs_name)).parent_path();
                if (!parent_path.empty() && !std::filesystem::exists(parent_path)) {
                    std::filesystem::create_directories(parent_path);
                }

                // Rename the file
                std::string new_path = path_append(torrent_dir, ipfs_name);
                std::filesystem::rename(file_path, new_path);
                log_message("Renamed " + original_path + " to " + ipfs_name);
            }
        }
    }

    // Rename the root directory if it's not already using the info_hash
    std::filesystem::path current_path(torrent_dir);
    if (current_path.filename().string() != info_hash) {
        std::string new_dir = path_append(save_path, info_hash);
        std::filesystem::rename(torrent_dir, new_dir);
        log_message("Renamed root directory to " + info_hash);
    }
}

// Function to rename files and folders back to original names
void rename_to_original_names(const std::string& save_path, const std::string& info_hash, torrent_handle& h) {
    std::string torrent_dir = path_append(save_path, info_hash);
    if (!std::filesystem::exists(torrent_dir)) {
        log_message("Error: Torrent directory does not exist: " + torrent_dir);
        return;
    }

    log_message("Renaming files in " + torrent_dir + " back to original names");

    // Get the torrent info to map IPFS names back to original names
    auto ti = h.torrent_file();
    if (!ti) {
        log_message("Error: Could not get torrent info");
        return;
    ...
    // Check if we have mappings for this torrent
    if (torrent_name_mappings.find(info_hash) == torrent_name_mappings.end() ||
        torrent_name_mappings[info_hash].empty()) {
        log_message("Error: No name mappings found for this torrent");
        return;
    }

    // First rename the root directory back to its original name if needed
    auto ti_name = ti->name();
    if (ti_name != info_hash) {
        std::string original_dir = path_append(save_path, ti_name);
        std::filesystem::rename(torrent_dir, original_dir);
        torrent_dir = original_dir;
        log_message("Renamed root directory back to " + ti_name);
    }

    // Process each file in the mappings
    for (const auto& mapping : torrent_name_mappings[info_hash]) {
        std::string ipfs_path = path_append(torrent_dir, mapping.second);
        std::string original_path = path_append(torrent_dir, mapping.first);

        if (std::filesystem::exists(ipfs_path)) {
            // Create parent directories if they don't exist
            ...
            std::filesystem::rename(ipfs_path, original_path);
            log_message("Renamed " + mapping.second + " back to " + mapping.first);
        }
    }
}
...
int print_peer_info(std::string& out, std::vector<lt::peer_info> const& peers, int max_lines)
{
    ...
    for (std::vector<peer_info>::const_iterator i = peers.begin(); i != peers.end(); ++i)
    {
        ...
        std::snprintf(str, sizeof(str),
            ...
            esc("32"), add_suffix(i->down_speed, "/s").c_str(), add_suffix(i->total_download).c_str(),
            ...
            esc("0"), temp, i->timed_out_requests, i->busy_requests, i->upload_queue_length,
...
void print_settings(int const start, int const num, char const* const type)
{
...
void set_torrent_params(lt::add_torrent_params& p)
{
    ...
    } else if (use_ipfs_naming) {
        p.save_path = save_path;
        log_message("Using IPFS-style naming for files and folders.");
    } else {
        p.save_path = save_path;
    }
...
void print_piece(lt::partial_piece_info const& pp, std::vector<lt::peer_info> const& peers, std::string& out)
{
    ...
    for (int j = 0; j < num_blocks; ++j)
    {
        ...
        if (pp.blocks[j].bytes_progress > 0 && pp.blocks[j].state == block_info::requested)
        {
...
CLIENT OPTIONS
  ...
  --ipfs                use IPFS-style naming for files and folders
...
#ifndef _WIN32
set_keypress::set_keypress(std::uint8_t const mode)
{
...
int main(int argc, char* argv[])
{
    ...
    if (enable_piece_cache || disable_original_storage)
    {
        try
        {
            ...
            if (disable_original_storage) {
                log_message("Original content storage disabled, using piece cache only");
            }
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (std::string(argv[i]) == "--ipfs") {
            use_ipfs_naming = true;
            continue;
        }
        ...
        switch (argv[i][1])
        {
            ...
            case 'a': allocation_mode = (arg == std::string("sparse")) ?
                lt::storage_mode_sparse : lt::storage_mode_allocate; break;
            case 'x':
                {
    ...
    std::thread resume_data_loader([&ses]
    {
        lt::error_code ec;
        std::string const resume_dir = disable_original_storage ?
            path_append(cache_root, ".resume") : path_append(save_path, ".resume");
        ...
        if (sleep_and_input(&c, refresh_delay))
        {
            ...
            do
            {
                ...
                if (c == 'm')
                {
                    ...
                    set_keypress echo(set_keypress::echo | set_keypress::canonical);
                ...
                if (c == 'R')
                {
                    std::vector<torrent_status> const torr = ses.get_torrent_status(
                        [](torrent_status const& st) { return st.need_save_resume; }, {});
                ...
                if (c == 'o' && h.is_valid())
                {
                    ...
                    {
                        h.set_piece_deadline(i, (static_cast<int>(i)+5) * 1000, torrent_handle::alert_when_available);
                    }
                }
                ...
                {
                    torrent_status const& ts = view.get_active_torrent();
                    if ((ts.flags & (lt::torrent_flags::auto_managed | lt::torrent_flags::paused)) == lt::torrent_flags::paused)
                    {
                ...
                {
                    torrent_status const& ts = view.get_active_torrent();
                    h.set_flags(~(ts.flags & lt::torrent_flags::auto_managed), lt::torrent_flags::auto_managed);
                    if ((ts.flags & lt::torrent_flags::auto_managed) && (ts.flags & lt::torrent_flags::paused))
                    {
                        h.resume();
                    }
                }
                ...
                // Toggle IPFS-style naming for torrent content
                if (c == 'Z' && h.is_valid())
                {
                    torrent_status const& ts = view.get_active_torrent();
                    std::string info_hash = to_hex(ts.info_hashes.get_best());
                    rename_to_ipfs_names(save_path, info_hash, h);
                }

                // Toggle back to original names
                if (c == 'N' && h.is_valid())
                {
                    torrent_status const& ts = view.get_active_torrent();
                    std::string info_hash = to_hex(ts.info_hashes.get_best());
                    rename_to_original_names(save_path, info_hash, h);
                }
...
[Z] rename all torrent content to IPFS-style names
[N] rename all torrent content back to original names
...
[7] toggle send buffers column                  [8] toggle local IP column
                ...
                for (file_index_t const i : ti->files().file_range())
                {
                    ...
                    std::snprintf(str, sizeof(str), "%s %7s p: %d ",
                        progress_bar(progress, file_progress_width,
                            pad_file ? col_blue : complete ? col_green : col_yellow,
                            '-', '#', title.c_str()).c_str(),
    ...
    quit = true;
    ses.pause();
    log_message("saving resume data");
    ...
    log_message("waiting for resume data [" + std::to_string(num_outstanding_resume_data) + "]");
...
#ifndef TORRENT_DISABLE_DHT
    log_message("saving session state");
    {
