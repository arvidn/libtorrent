...
def __lldb_init_module(debugger: lldb.SBDebugger, internal_dict: Dict) -> None:
    ...
    debugger.HandleCommand(
        'type summary add -x "^libtorrent::span<.+>$" -F '
    ...
    debugger.HandleCommand(
        'type summary add -x "^libtorrent::bitfield$" -F '
...
def pairs(lst: List[Any]) -> Iterator[List[Any]]:
...
def print_endpoint(valobj: lldb.SBValue, internal_dict: Dict) -> str:
    ...
    if family == 2:
        a = (
            union.GetChildMemberWithName("v4")
            .GetChildMemberWithName("sin_addr")
            .GetData()
            .uint8s
        )
        ...
        p = swap16(
            union.GetChildMemberWithName("v4")
            .GetChildMemberWithName("sin_port")
            .GetValueAsUnsigned()
        )
        return "{}:{}".format(addr, p)
    else:
        a = (
            union.GetChildMemberWithName("v6")
            .GetChildMemberWithName("sin6_addr")
            .GetData()
            .uint8s
        )
        p = swap16(
            union.GetChildMemberWithName("v6")
            .GetChildMemberWithName("sin6_port")
            .GetValueAsUnsigned()
        )
