...
#include "libtorrent/aux_/string_util.hpp" // for string_ends_with
...
TORRENT_TEST(udp_tracker_i2p_no_dns_leak)
{
	struct sim_config : sim::default_config
	{
		...
		chrono::high_resolution_clock::duration hostname_lookup(asio::ip::address const& requestor,
			...
			boost::system::error_code& ec) override
		{
			if (lt::aux::string_ends_with(hostname, ".i2p"))
				++i2p_lookups;
			return default_config::hostname_lookup(requestor, hostname, result, ec);
		}
	};
	...
	params.flags &= ~lt::torrent_flags::auto_managed;
	params.flags &= ~lt::torrent_flags::paused;
	params.trackers.push_back("udp://tracker.i2p:6969/announce");
	params.save_path = save_path(0);
	ses->async_add_torrent(params);
...
void test_peer_hostname_resolve(peer_hostname_flags_t const flags)
{
	...
	http.register_handler(
		"/announce", [](std::string, std::string, std::map<std::string, std::string>&) {
			...
			std::string const peer_dict =
				"d2:ip" + std::to_string(peer_host.size()) + ":" + peer_host + "4:porti6881ee";
	...
	bool const proxy_hostnames_enabled =
		bool(flags & no_proxy_configured) ? true : bool(flags & proxy_hostnames);
	TEST_EQUAL(
		network_cfg.peer_lookups > 0, !(proxy_hostnames_enabled && bool(flags & configure_proxy)));
}

// regression test: without proxy_hostnames, a hostnamed peer from the
// tracker is still resolved normally.
TORRENT_TEST(peer_hostname_resolved_without_proxy_hostnames) { test_peer_hostname_resolve({}); }
