...
namespace {
	...
	// torrent::abort(), which clears the paused flag directly, without going
	// through do_resume(), so these timers rely on is_aborted() (in addition
	// to is_paused()) to know they're no longer live -- and abort() itself
	// must flush the live segment into the accumulators before that happens,
	// the same way do_pause() does.
...
// clears the have_all flag via set_have_all(), which is_seed() depends on,
// so the live seeding segment must be flushed into the seeding timer before
// that happens, and re-anchored once the recheck completes and confirms the
// torrent is a seed again.
