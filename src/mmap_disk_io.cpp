...
// ------- mmap_disk_io ------
	...
	status_t mmap_disk_io::do_job(aux::job::hash& a, aux::mmap_disk_job* j)
	{
		...
		// block where we don't have one, fall through to the existing path
		aux::mmap_storage::precomputed_piece pc =
			v2 ? j->storage->take_precomputed_v2(a.piece) : aux::mmap_storage::precomputed_piece{};
		...
		for (int i = 0; i < blocks_to_read; ++i)
		{
			...
			bool const have_precomputed_v2 =
				v2_block && i < pc.present.size() && pc.present.get_bit(i);
			...
			if (v2_block) a.block_hashes[i] = have_precomputed_v2 ? pc.hashes[i] : h2.final();
