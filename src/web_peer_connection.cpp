...
#include "libtorrent/aux_/string_util.hpp" // for format_host_header
...
web_peer_connection::web_peer_connection(peer_connection_args& pack, aux::web_seed_t& web)
	...
	, m_extra_headers(web.extra_headers)
{
...

	{
		...
		if (m_port == -1 && protocol == "http") m_port = 80;
	...
	else
	{
		...
		{
			m_path += escape_file_path(m_ti->layout(), file_index_t(0));
		}
...
int web_peer_connection::timeout() const
{
...
void web_peer_connection::add_headers(std::string& request, bool const using_proxy) const
{
	request += "Host: ";
...
bool web_peer_connection::in_handshake() const { return m_server_string.empty(); }

void web_peer_connection::on_sent(error_code const& error, std::size_t bytes_transferred)
{
	INVARIANT_CHECK;
