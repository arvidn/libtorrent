...
namespace {
	...
	torrent_handle session_handle::add_torrent(add_torrent_params&& params)
	{
		...
		TORRENT_ASSERT_PRECOND(!is_absolute(params.part_file_dir));
...
#endif
		error_code ec;
		auto r = sync_call_ret<torrent_handle>(&session_impl::add_torrent, std::move(params), std::ref(ec));
	...
	void session_handle::async_add_torrent(add_torrent_params&& params)
	{
		...
		p->save_path = absolute(p->save_path);
		...
		, const char* net_interface, int flags)
	{
		...
		interfaces_str = aux::print_endpoint(tcp::endpoint(make_address(net_interface, ec), std::uint16_t(port_range.first)));
		if (ec) return;
