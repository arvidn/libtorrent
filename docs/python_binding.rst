...
``setup.py``::
	...
	python setup.py build_ext --toolset=msvc-14.2

For a full list of options you can customize::
...
``distutils`` normally works. Consider::
	...
	python setup.py build_ext --optimization=space
...
should run the steps inline with ``build_ext``::
	...
	python setup.py build_ext --optimization=space install
