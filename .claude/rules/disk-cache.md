...
**Fences and the cache (`pread_disk_io`):** the general fence contract is in `.claude/rules/disk-io.md`; this is how `pread_disk_io` honours it with a write-back cache.

- **Writes during a fence are queued, not aborted.** `async_write` calls `is_blocked()`, which takes ownership of the job and parks it in the fence's blocked queue. When the fence lowers, `disk_job_fence::job_complete` calls the `repost` callback (still under the fence mutex) once per blocked job in posting order; `add_completed_jobs_impl` inserts each reposted write into the cache there, and serves reposted reads from the cache (a normal disk read would return stale data, since the superseding write is still buffered). Because the resume runs under the fence mutex, a job posted concurrently is ordered behind the whole backlog. Work that can re-enter the fence (hasher kick, flush, completions) is deferred to after the mutex is released.
- **`async_read` never consults the cache; `add_job` is the single funnel.** `async_read` only allocates a `job::read` (carrying the request verbatim, including an unaligned, block-spanning start) and hands it to `add_job`. `add_job` runs `is_blocked()` first, so a read posted while a fence is up is parked like any other job and cannot bypass the fence to serve stale data ahead of a queued write. The cache is consulted exactly once, by `prepare_read()`, and only after the job is past the fence gate: in `add_job` when the fence is down, or in the `repost` callback when the fence lowers. Both call `prepare_read()`; reposted jobs must **not** re-enter `is_blocked()` (a stacked fence still raises `m_has_fence`, so it would wrongly re-park them behind a later fence), which is why the consult is shared *after* the gate rather than by routing reposts back through `add_job`. `prepare_read()` mutates the job in place, so the single outstanding-job accounting `is_blocked()`/`job_complete()` set up is preserved.
- **Every write is counted; writes during a fence are queued, not aborted.** `async_write` always routes through `is_blocked()` (never a fast path around it), so the write counts as an outstanding job on the storage whether or not a fence is up -- a fence then waits for buffered writes and their in-flight flush, exactly like a read or hash (see `.claude/rules/disk-io.md`). With no fence up, `is_blocked()` marks the job in-progress and increments the count, and the write is inserted into the cache inline; it stays counted until it is flushed and completed (or aborted). With a fence up, `is_blocked()` instead takes ownership of the job and parks it in the fence's blocked queue. When the fence lowers, `disk_job_fence::job_complete` calls the `repost` callback (still under the fence mutex) once per blocked job in posting order; `add_completed_jobs_impl` inserts each reposted write into the cache there, and serves reposted reads from the cache (a normal disk read would return stale data, since the superseding write is still buffered). Because the resume runs under the fence mutex, a job posted concurrently is ordered behind the whole backlog. Work that can re-enter the fence (hasher kick, flush, completions) is deferred to after the mutex is released.
- **Reads racing the unblock.** `async_read`'s lock-free cache probe may run just before the backlog's writes land. `add_job` re-checks the cache once the fence is observed down, so a read is never dispatched to disk for a block whose write is sitting buffered.
- **Forward progress (`m_fence_flush`).** A fence waits for the storage's outstanding jobs (including buffered writes) to reach zero, but nothing else flushes a partial piece under the watermark — so a raised fence hands its storage to a generic thread to flush (`flush_fenced_storages`), draining the writes it is waiting on. `add_fence_job` does this when a fence is raised over outstanding work (`fence_post_none`).
- **Stacked fences.** When a fence lowers and reposts writes that sit ahead of a still-raised second fence, those partial-piece writes are now in the cache with no flush trigger. `add_completed_jobs_impl` detects a completed fence whose storage still `has_fence()` and re-arms `m_fence_flush` for it, so the next fence's wait can also make progress. (With no generic threads it flushes inline.)
- **Unaligned reads and `job::partial_read`.** A peer read request whose start is not block-aligned can span two blocks. `prepare_read()` probes both with `disk_cache::get2`: if both blocks are in the cache it serves the whole request; if neither is, the job stays a `job::read` and `do_job(read)` reads the whole span from disk; if exactly one block is in the cache, it copies that side into the buffer and rewrites the job in place into a `job::partial_read` whose `offset`/`buffer_size`/`buffer_offset` describe just the missing side. `do_job(partial_read)` then reads only that side from disk into the rest of the buffer. Because `prepare_read()` runs past the fence gate against the *current* cache, the cached side it copies is never stale, and a `partial_read` therefore reads only from disk and never re-consults the cache. `prepare_read()` is the only place `partial_read` jobs are created -- they are never posted directly.
