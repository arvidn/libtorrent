...
// polls h's progress (rather than waiting for a single alert) until it
// reaches 100%, so it also covers multi-pass completions like a recheck.
// bounded by a 30s stall backstop (no progress change) and a 200s absolute
// backstop; fails the test if the torrent never finishes.
EXPORT void wait_for_complete(lt::session& ses, lt::torrent_handle h);
