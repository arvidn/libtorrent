...
struct obs : dht::dht_observer
{
	...
	int get_listen_port(aux::transport, aux::listen_socket_handle const& s) const override
...
TORRENT_TEST(signing_oversized_salt)
{
...
TORRENT_TEST(rate_limit_non_positive)
{
	...
	for (int const limit : limits)
	{
		...
		test_rate_limit(sett, [](lt::dht::socket_manager& sm) { TEST_CHECK(!sm.has_quota()); });
	}
}
