...
// identifies the most recently started web server's stopped-announces log,
// independent of its (only known after spawn) port. Each test process runs
// in its own working directory, so a plain counter is enough to keep
// concurrent test runs from clobbering each other's file.
int web_server_instance = 0;
}

std::string web_server_stopped_announces_path()
{
	return "stopped_announces." + std::to_string(web_server_instance);
}
	...
	bool ssl, bool chunked_encoding, bool keepalive, int min_interval, bool expect_host_header)
{
	...
	for (auto const& python_exe : get_python())
	{
		...
		std::snprintf(buf,
			sizeof(buf),
			"%s .." SEPARATOR "web_server.py %d %d %d %d %s %d",
			python_exe.c_str(),
			chunked_encoding,
			ssl,
			keepalive,
