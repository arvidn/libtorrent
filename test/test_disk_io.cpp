...
namespace {
void disk_io_test_suite(lt::disk_io_constructor_type disk_io, disk_test_mode_t const flags, int piece_size)
{
	lt::io_context ios;
	lt::counters cnt;
...
{
	disk_io_test_suite(&lt::mmap_disk_io_constructor, test_mode::v1 | test_mode::v2, 300);
}

TORRENT_TEST(test_mmap_disk_io)
{
	disk_io_test_suite(&lt::mmap_disk_io_constructor, test_mode::v1 | test_mode::v2, 0x8000);
}
#endif

TORRENT_TEST(test_posix_disk_io)
{
	disk_io_test_suite(&lt::posix_disk_io_constructor, test_mode::v1 | test_mode::v2, 0x8000);
}

TORRENT_TEST(test_pread_disk_io)
{
	disk_io_test_suite(&lt::pread_disk_io_constructor, test_mode::v1 | test_mode::v2, 0x8000);
}
