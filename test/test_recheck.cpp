...
// on_resume_data_checked() (as constructed by this test): it dispatches a
// verify_piece() job for the unfinished piece here, and start_checking()
// dispatches its own hash for the same piece shortly after.
//
// The unfinished piece is deliberately the *last* piece start_checking()
// will check. piece_passed()/on_piece_hashed() are idempotent (see the
// guard at the top of each), so whichever of the two competing hash jobs
// completes second is a no-op rather than corrupting file_progress or the
// picker -- but on_piece_hashed()'s no-op path must still fall through to
// its checking-loop continuation (dispatch the next piece, or finish the
// check). Racing the last piece means that continuation is what calls
// files_checked(); skipping it here (rather than for some earlier piece,
// where other outstanding jobs would mask the bug) would stall the check
// forever, caught by wait_for_complete()'s timeout.
TORRENT_TEST_DISK_IO(recheck_unfinished_piece_vs_start_checking)
{
	...
	piece_index_t const last_piece(num_pieces - 1);
	...
	// resume data *also* claims the last piece's only block is already
	// written (but not yet hash-checked). Genuine resume data would never
	// pair this with a truncated have_pieces bitfield (see the comment
	// above), but nothing stops an add_torrent_params constructed this way,
	// directly or from an external resume file, from reaching
	// on_resume_data_checked().
	bitfield const blocks(1, true);
	param.unfinished_pieces[last_piece] = blocks;
	...
	// covering pieces 1-3 should bring this to 100% on its own.
