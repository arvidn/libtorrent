...
void test_part_file(lt::storage_mode_t const storage_mode, part_file_flag_t const flags)
{
	...
	delete_dirs("temp_storage_part_file");

	std::vector<lt::create_file_entry> fs;
	fs.emplace_back("temp_storage_part_file/test1.tmp", 17);
	fs.emplace_back("temp_storage_part_file/test2.tmp", 612);
	...
	std::string const torrent_dir = combine_path(download_dir, "temp_storage_part_file");
	std::string const part_file_dir = (flags & custom_path)
		? combine_path(cwd, "part_file_dir")
		: std::string();
	++test_index;
