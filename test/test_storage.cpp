...
TORRENT_TEST_DISK_IO(rename_valid_symlink)
{
	...
	fs.emplace_back("symlink_rename_valid/sub/link", 0, file_storage::flag_symlink, 0, "sub/data");
	lt::add_torrent_params const atp = load_symlink_torrent(std::move(fs));
...
{
	std::string const test_path = current_path();
	std::string const new_test_path = absolute("symlink_move_self_dest");
...
{
	std::string const test_path = current_path();
	std::string const new_test_path = absolute("symlink_move_storage_dest");
...
TORRENT_TEST(fastresume_spanning_piece_missing_file)
{
	...
	aux::stat_cache stat;
	storage_error se;
	lt::renamed_files const rf;
	lt::filenames const fn(fs, rf);
	bool const valid = aux::verify_resume_data(rd, links, fn, priorities, stat, save_path, se);
...
TORRENT_TEST(pread_disk_io_part_file)
{
	for (auto storage_mode : { storage_mode_sparse, storage_mode_allocate })
	{
		for (auto flags : {part_file_flag_t{}, custom_path})
		{
			test_part_file<pread_storage>(storage_mode, flags);
		}
	}
}
