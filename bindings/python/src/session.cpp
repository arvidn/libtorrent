...
namespace boost {
	...
	template <>
	inline lt::alert const volatile* get_pointer(lt::alert const volatile* p)
	{
		return p;
	}
}
...
namespace {
	...
	dict make_dict(lt::settings_pack const& sett)
	{
		...
		for (int i = settings_pack::string_type_base;
			 i < settings_pack::max_string_setting_internal;
			 ++i)
		{
		...
		for (int i = settings_pack::int_type_base; i < settings_pack::max_int_setting_internal; ++i)
		{
			char const* name = name_for_setting(i);
			if (name[0] != '\0' && sett.has_val(i)) ret[name] = sett.get_int(i);
		}

		for (int i = settings_pack::bool_type_base; i < settings_pack::max_bool_setting_internal;
			 ++i)
		{
			char const* name = name_for_setting(i);
	...
	std::shared_ptr<lt::session> make_session(boost::python::dict sett, session_flags_t const flags)
	{
		session_params p;
#if TORRENT_ABI_VERSION <= 2
		if (!(flags & lt::session::add_default_plugins)) p.extensions.clear();
...
//		python_deprecated("add_default_plugins flag is deprecated");
	...
	dict default_settings_wrapper() { return make_dict(default_settings()); }
	...
	{
		notify_callback_wrapper(object o)
			: m_cb(o)
		{}
...
void dict_to_add_torrent_params(dict params, add_torrent_params& p)
{
	...
	for (int i = 0; i < len; i++)
	{
		...
		if (key == "ti" && value != boost::python::object())
		{
			...
			p.ti = std::make_shared<torrent_info>(extract<torrent_info const&>(value));
			continue;
		}
		...
		else if (key == "info_hashes")
		{
			if (boost::python::len(value) == sha1_hash::size())
			{
				p.info_hashes = info_hash_t(sha1_hash(bytes(extract<bytes>(value)).arr.data()));
			}
			else if (boost::python::len(value) == sha256_hash::size())
			{
				p.info_hashes = info_hash_t(sha256_hash(bytes(extract<bytes>(value)).arr.data()));
			}
			else
			{
		...
		else if (key == "http_seeds")
		{
			...
			p.http_seeds = extract<decltype(add_torrent_params::http_seeds)>(value);
			continue;
		}
#endif
		else if (key == "dht_nodes")
		{
			p.dht_nodes = extract<std::vector<std::pair<std::string, int>>>(value);
			continue;
		}
		...
		{
			PyErr_SetString(PyExc_KeyError, ("unknown name in torrent params: " + key).c_str());
			throw_error_already_set();
		}
	}
}
...
namespace {
	...
	torrent_handle wrap_add_torrent(lt::session& s, lt::add_torrent_params const& p)
	{
		...
		if (p.ti) atp.ti = std::make_shared<torrent_info>(*p.ti);
	...
	void wrap_async_add_torrent(lt::session& s, lt::add_torrent_params const& p)
	{
		...
		if (p.save_path.empty())
		{
			PyErr_SetString(PyExc_ValueError, "save_path must be set in add_torrent_params");
			throw_error_already_set();
		}
	...
	void set_alert_fd(lt::session& s, std::intptr_t const fd)
	{
		...
		if (sock != INVALID_SOCKET
			&& ::getsockopt(sock, SOL_SOCKET, SO_ERROR, (char*)&res, &res_size) == 0)
		{
	...
	bool wrap_pred(object pred, torrent_status const& st) { return pred(st); }
	...
	void set_peer_class(lt::session& ses, peer_class_t const pc, dict info)
	{
		...
		for (; i != end; ++i)
		{
			...
			{
				PyErr_SetString(
					PyExc_KeyError, ("unknown name in peer_class_info: " + key).c_str()
				);
				throw_error_already_set();
			}
		}
	...
	{
		if (key.arr.size() != 32) throw std::invalid_argument("key has wrong size, should be 32");
	...
	)
	{
		...
		++seq;
		dht::signature sign = sign_mutable_item(
			buf,
			salt,
	...
	void dht_put_mutable_item(
		lt::session& ses, bytes private_key, bytes public_key, bytes data, bytes salt
	)
	{
		...
		ses.dht_put_item(
			key,
			[pk = std::move(public_key.arr),
			 sk = std::move(private_key.arr),
			 d = std::move(data.arr)](
	...
	add_torrent_params read_resume_data_wrapper0(bytes const& b) { return read_resume_data(b.arr); }
	...
	int find_metric_idx_wrap(char const* name) { return lt::find_metric_idx(name); }
	...
	session_params read_session_params_entry(dict e, save_state_flags_t flags)
	{
	...
	session_params read_session_params_buffer(bytes const& bytes, save_state_flags_t flags)
	{
		return lt::read_session_params(bytes.arr, flags);
	}

	bytes write_session_params_bytes(session_params const& sp, save_state_flags_t flags)
	{
	...
	struct dict_to_settings
	{
		dict_to_settings()
		{
			converter::registry::push_back(&convertible, &construct, type_id<settings_pack>());
		}

		static void* convertible(PyObject* x) { return PyDict_Check(x) ? x : nullptr; }

		static void construct(PyObject* x, converter::rvalue_from_python_stage1_data* data)
		{
			void* storage =
				((converter::rvalue_from_python_storage<lt::settings_pack>*)data)->storage.bytes;
	...
	torrent_handle (lt::session_handle::*find_torrent0)(sha1_hash const&) const =
		&lt::session_handle::find_torrent;
	torrent_handle (lt::session_handle::*find_torrent1)(sha256_hash const&) const =
		&lt::session_handle::find_torrent;
...
void bind_session()
{
	...
	class_<session_status>("session_status", no_init)
		...
		.add_property(
			"active_requests",
			make_getter(&session_status::active_requests, return_value_policy<return_by_value>())
		)
...
#define PROP(val)                                                                                  \
	...
	def("read_session_params",
		&read_session_params_entry,
		(arg("dict"), arg("flags") = save_state_flags_t::all()));
	def("read_session_params",
		&read_session_params_buffer,
		(arg("buffer"), arg("flags") = save_state_flags_t::all()));
	def("write_session_params",
		&lt::write_session_params,
		(arg("entry"), arg("flags") = save_state_flags_t::all()));
	def("write_session_params_buf",
		&write_session_params_bytes,
		(arg("buffer"), arg("flags") = save_state_flags_t::all()));
		...
		scope s =
			class_<lt::session, boost::noncopyable>("session", no_init)
...
#if TORRENT_ABI_VERSION == 1
				.def(init<fingerprint, session_flags_t, alert_category_t>(
					(arg("fingerprint") = fingerprint("LT", 0, 1, 0, 0),
					 arg("flags") =
						 lt::session::start_default_features | lt::session::add_default_plugins,
				...
				.def(
					...
					allow_threads(&lt::session::post_torrent_updates),
					arg("flags") = 0xffffffff
				)
...
#if TORRENT_ABI_VERSION == 1
				.def("add_dht_router", depr(&add_dht_router), (arg("router"), "port"))
#endif // TORRENT_ABI_VERSION
				...
				.def(
					"dht_announce",
					&dht_announce,
					(arg("info_hash"), arg("port") = 0, arg("flags") = 0)
				)
				...
				.def(
					"remove_torrent", allow_threads(&lt::session::remove_torrent), arg("option") = 0
				)
#if TORRENT_ABI_VERSION == 1
				...
				.def(
					"get_torrent_status",
					&get_torrent_status,
					(arg("session"), arg("pred"), arg("flags") = 0)
				)
				.def(
					"refresh_torrent_status",
					&refresh_torrent_status,
					(arg("session"), arg("torrents"), arg("flags") = 0)
				)
				...
				.def(
					...
					depr(&lt::session::set_local_download_rate_limit)
				)
				...
				.def(
					...
					depr(&lt::session::set_max_half_open_connections)
				)
	...
	entry (*write_torrent_file0)(add_torrent_params const&, write_torrent_flags_t) =
		&write_torrent_file;
