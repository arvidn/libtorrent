...
namespace {
	...
	long get_hash(sha1_hash const& s) { return std::hash<sha1_hash>{}(s); }

	bytes sha1_hash_bytes(const sha1_hash& bn) { return bytes(bn.to_string()); }

	std::shared_ptr<sha1_hash> bytes_constructor(bytes s)
	{
		if (s.arr.size() < 20) throw std::invalid_argument("short hash length");
		if (s.arr.size() > 20)
	...
	std::shared_ptr<sha1_hash> string_constructor(string_view const& sv)
	{
		...
		std::string s(sv);
		if (s.size() < 20) throw std::invalid_argument("short hash length");
		if (s.size() > 20) python_deprecated("long hash length. this will work, but is deprecated");
